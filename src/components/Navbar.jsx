      ...
      <nav className="flex justify-between items-center w-full px-margin-mobile md:px-margin-desktop py-4 max-w-container-max mx-auto">
        ...
        <div className="flex items-center gap-2">
          {isAuthenticated ? (
            ...
            <>
              {/* CTA utama: pre-register (daftar tunggu). Registrasi mandiri masih ditutup. */}
              <Link
                to="/register"
                className="hidden sm:inline-flex px-5 py-2 rounded-full bg-primary text-on-primary text-sm font-semibold hover:bg-surface-tint transition-colors cursor-pointer"
              >
                Daftar Gratis
              </Link>
              <Link
                to="/auth"
                className="px-5 py-2 rounded-full border border-outline-variant text-on-surface text-sm font-semibold hover:bg-surface-container-low transition-colors cursor-pointer"
              >
                Masuk
              </Link>
            </>
